...
def orchestrate():
    ...
    def ingest_cdc():
        ws = WorkspaceClient(
        host="https://dbc-17120271-1eb0.cloud.databricks.com",
        token=os.environ.get("DBT_TOKEN")
        )

        job_trigger = ws.jobs.run_now(job_id="1045384885093915")
    ...
    @task.bash
    def clean_target():
        return "rm -rf /opt/airflow/wallmart_project/target && rm -rf /opt/airflow/wallmart_project/logs"

    @task.bash
    def source_freshness():
        # Manually set the working directory using the 'cd' command before running the dbt command
        return "cd /opt/airflow/wallmart_project && dbt source freshness"
    

    silver_technical = BashOperator(
        task_id='silver_technical',
        cwd='/opt/airflow/wallmart_project',
